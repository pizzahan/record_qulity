...
def process(configer, beginTime, endTime):
    ...
    try:
        ...
        srcSql = ''' select a.dphone,c.dirid,c.filename,a.id,a.cid,a.prjid,b.duration,d.memo,e.memo,a.content from pre_cc_biztrack a,pre_cc_call b,pre_cc_recfile c,pre_cc_corp d,pre_cc_project e 
        ...
        if effect_rows > 0:
            ...
            for row in rows:
                ...
                if not rec.checkExists(destFile):
                    ...
                    destSql = ''' insert into mm_record_quality(mobile,file_name,status,src_id,cid,prjid,duration,company_name,project_name,content) value ('{0}','{1}','RDY',{2},{3},{4},{5},'{6}','{7}','{8}') '''.format(
                        mobile, destFile, srcId, cid, prjid, duration, company_name, project_name, content)
